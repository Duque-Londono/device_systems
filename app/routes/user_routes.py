...
@router.get(
    ...
    description=(
        ...
        "e `is_active` (true/false); ambos pueden combinarse. "
...
def get_users(filters: FiltersDep):
...
def replace_user(data: UserUpdate, user: UserDep):
    # UserDep ya garantizó la existencia (404 si no existe)
    try:
        return user_service.update_user_full(user["id"], data)
    except ValueError as error:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(error))
...
def update_partial(data: UserPatch, user: UserDep):
    # UserDep ya garantizó la existencia (404 si no existe)
    try:
        return user_service.update_user_partial(user["id"], data)
    except ValueError as error:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(error))
...
def delete_user(user: UserDep, config: ConfigDep, _auth: ApiKeyDep, response: Response):
    # UserDep ya garantizó la existencia (404 si no existe)
